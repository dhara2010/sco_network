    ...
    <div className="min-h-screen bg-white pb-16">
      ...
      <div className="bg-[#1056A5] text-white py-24 px-4 text-center relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          >
            <button className="bg-[#fdb813] text-[#0B2B5B] font-bold text-[15px] px-8 py-3.5 rounded-lg hover:bg-[#e2a411] transition-colors shadow-md flex items-center gap-2">
              <Download size={18} />
              Latest Report 2024-25
            </button>
            <button className="bg-white/10 border border-white/30 text-white font-bold text-[15px] px-8 py-3.5 rounded-lg hover:bg-white/20 transition-colors backdrop-blur-sm">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {categories.map((cat) => (
              <button
                ...
                className={`px-5 py-2.5 rounded border text-[13px] font-semibold whitespace-nowrap transition-colors ${
                  activeTab === cat 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {loading ? (
            ...
            reports.map((report, index) => (
              <div key={report._id} className="bg-white rounded-[20px] shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05),0_10px_20px_-2px_rgba(0,0,0,0.02)] border border-gray-100 p-8 flex flex-col items-center relative hover:-translate-y-1 transition-transform duration-300 group">
                
                {/* Ribbon for newest report */}
                {index === 0 && activeTab === 'All Years' && (
                  <div className="absolute top-0 right-0 bg-[#fef5d9] p-3 rounded-bl-3xl rounded-tr-[20px]">
                     <Star size={20} className="text-[#fdb813] fill-current" />
                  </div>
                )}

                <div className="w-16 h-16 rounded-full bg-blue-50 flex items-center justify-center text-[#1056A5] mb-5 group-hover:scale-110 transition-transform">
                  <FileText size={28} strokeWidth={1.5} />
                </div>
                
                <h3 className="text-[42px] font-extrabold text-[#0B2B5B] leading-none mb-2">{report.reportYear}</h3>
                <p className="text-[13px] font-medium text-gray-500 text-center mb-8 h-10">{report.title}</p>
                
                <a 
                  href={report.reportFile} 
                  target="_blank" 
                  rel="noreferrer"
                  className="mt-auto w-full py-3 rounded-xl bg-[#F8F9FA] text-[#0B2B5B] font-bold text-[14px] hover:bg-[#1056A5] hover:text-white transition-colors border border-gray-100 flex items-center justify-center gap-2"
                >
          ...
          <div className="bg-[#1056A5] rounded-[20px] shadow-lg p-8 flex flex-col items-center justify-center text-center relative overflow-hidden group">
             ...
             <div className="relative z-10 flex flex-col items-center w-full h-full">
               ...
               <button className="mt-auto w-full py-3 rounded-xl bg-[#fdb813] text-[#0B2B5B] font-bold text-[14px] hover:bg-[#e2a411] transition-colors flex items-center justify-center gap-2 shadow-sm">
        ...
        <div className="mt-20 bg-[#fffdf5] border border-[#fef0c7] rounded-[24px] p-8 md:p-12 flex flex-col lg:flex-row items-center justify-between gap-8">
          ...
          <div className="w-full lg:w-auto flex-1 max-w-md">
            <div className="flex bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm">
              ...
              />
              <button className="bg-[#0B2B5B] text-white px-8 py-3 rounded-lg font-bold text-[14px] hover:bg-[#0a1a3a] transition-colors">
                Subscribe
              </button>
            </div>
          </div>
