...
const CommitteeMembersPage = () => {
  ...
  const currentMembers = members.filter(m => m.designation === activeTab);
          ...
          {displayedMembers.map(member => (
            <div key={member._id} className="bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-xl transition-all duration-300 border border-gray-100 p-7 flex flex-col items-center">
              ...
              <div className="w-full space-y-4 text-[13px] font-medium text-[#4B5563] mb-7 flex-grow px-1">
                ...
                {member.email && (
                  <div className="flex items-center gap-3.5">
                    <Mail size={16} className="text-[#0B2B5B] shrink-0" />
              ...
              {/* View Profile Button */}
              <Link to={`/member/${member._id}`} className="mt-6 w-full py-2.5 rounded-xl bg-[#F8F9FA] text-[#0B2B5B] font-bold text-[14px] hover:bg-[#1056A5] hover:text-white transition-colors border border-gray-100 flex items-center justify-center gap-2">
            ...
            {[...Array(totalPages)].map((_, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-[14px] font-semibold transition-colors ${isCurrent ? 'bg-[#0B2B5B] text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'
                    }`}
                >
                  {pageNumber}
                </button>
