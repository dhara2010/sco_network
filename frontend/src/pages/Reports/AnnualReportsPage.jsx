  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        let filtered = data.filter(r => r.status === 'Approved' || r.status === 'Published');
        if (searchYear) {
          filtered = filtered.filter(r => r.year === searchYear);
        }
        if (searchTitle) {
          filtered = filtered.filter(r => r.title.toLowerCase().includes(searchTitle.toLowerCase()));
        }
        setReports(filtered);
      } catch (err) {
