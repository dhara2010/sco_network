  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        ...
        setChapters(data.filter(c => c.status === 'approved' && c.latitude && c.longitude));
      } catch (err) {
        console.error('Failed to fetch chapters for map', err);
      } finally {
        setLoading(false);
      }
    };
            ...
            <Marker key={chapter._id} coordinates={[chapter.longitude, chapter.latitude]}>
              ...
              >
                ...
                <circle
                  ...
                  transform={hoveredChapter === chapter._id ? "scale(1.5)" : "scale(1)"}
                  filter={hoveredChapter === chapter._id ? "url(#glow)" : ""}
                />

                {/* Tooltip implementation inside SVG */}
                {hoveredChapter === chapter._id && (
                  <g transform="translate(0, -20)" className="animate-in fade-in slide-in-from-bottom-2 duration-200 z-50">
                    ...
                    />
                    <polygon points="-6,6 6,6 0,12" fill="rgba(255, 255, 255, 0.95)" />
                    <polyline points="-6,6 0,12 6,6" fill="none" stroke="#115fc6" strokeWidth="1.5" />
                    ...
                    >
                      {chapter.chapterName || chapter.cityName}
                    </text>
                  </g>
                )}
              </g>
