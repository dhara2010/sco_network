...
import {
  ...
  CheckCircle, Clock, Search, ChevronRight, XCircle, Award, FolderKanban
} from 'lucide-react';
import { membersData, projectsData, reportsData, chaptersData } from '../data/staticData';
...
const DashboardHome = () => {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      // Group by chapters for leaderboard
      const chapterCounts = membersData.reduce((acc, member) => {
        if (member.city) {
      ...
      // Approved reports for percentage
      const approvedReports = reportsData.filter(r => r.status === 'Published').length;
      const reportApprovalRate = reportsData.length > 0 ? Math.round((approvedReports / reportsData.length) * 100) : 100;

      setData({ 
        hero: { 
          totalMembers: membersData.length, memberGrowth: 5, 
          totalProjects: projectsData.length, projectGrowth: 8, 
          totalReports: reportsData.length, reportGrowth: 2
        }, 
        pending: { 
        ...
        chartData: [
          {name: 'Jan', projects: 10, reports: 5}, 
          {name: 'Feb', projects: Math.round(projectsData.length / 2), reports: Math.round(reportsData.length / 2)},
          {name: 'Mar', projects: projectsData.length, reports: reportsData.length}
        ], 
        leaderboards: { 
          topChapters: sortedChapters.length > 0 ? sortedChapters : [{chapter: 'Ahmedabad', count: 12}], 
          topContributors: membersData.slice(0, 3).map((m, idx) => ({ member: m, count: 100 - (idx * 20) })) 
        } 
      });
      setLoading(false);
    }, 400);
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { title: "Members", count: pending.members, link: "/members", icon: Users },
                { title: "Projects", count: pending.projects, link: "/projects", icon: Folder },
                { title: "Reports", count: pending.reports, link: "/reports", icon: FileText },
                { title: "Activities", count: pending.activities, link: "/activities", icon: Activity }
              ].map((item, idx) => (
